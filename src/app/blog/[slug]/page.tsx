...
export default async function PostPage({ params }: Props) {
  ...
  return (
    <article className="px-5 md:px-8 py-10 max-w-2xl mx-auto">
      ...
      <div className="
        max-w-none font-sans text-base text-ink-secondary leading-[1.85]
        [&>p]:mb-6 [&>p]:text-base [&>p]:leading-[1.85]
        [&>h2]:font-serif [&>h2]:font-bold [&>h2]:text-ink [&>h2]:text-xl [&>h2]:mt-10 [&>h2]:mb-4
        [&>h3]:font-serif [&>h3]:font-bold [&>h3]:text-ink [&>h3]:text-lg [&>h3]:mt-8 [&>h3]:mb-3
        [&>ul]:mb-6 [&>ul]:pl-5 [&>ul>li]:mb-2
        [&>ol]:mb-6 [&>ol]:pl-5 [&>ol>li]:mb-2
        [&>blockquote]:border-l-2 [&>blockquote]:border-cc-green [&>blockquote]:pl-4 [&>blockquote]:text-ink-muted [&>blockquote]:italic [&>blockquote]:my-6
        [&>img]:rounded [&>img]:my-8 [&>img]:w-full
        [&_a]:text-cc-green [&_a]:no-underline [&_a:hover]:underline
        [&_strong]:text-ink [&_strong]:font-semibold
        [&_sup]:text-xs [&_sup>a]:text-cc-green [&_sup>a]:font-semibold [&_sup>a]:no-underline [&_sup>a:hover]:underline
      ">
      ...
      {post.sources && post.sources.length > 0 && (
        <div className="mt-12 pt-8 border-t border-surface-border">
          <div className="text-xs font-sans font-bold uppercase tracking-widest text-ink-muted mb-4">
            Sources
          </div>
          <ol className="space-y-2">
            {post.sources.map((source, i) => (
              <li
                key={i}
                id={`source-${i + 1}`}
                className="flex items-baseline gap-2 text-xs font-sans text-ink-muted"
              >
                <span className="flex-shrink-0 font-semibold text-ink-faint">[{i + 1}]</span>
                <a
                  href={source.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-cc-green hover:underline leading-relaxed"
                >
                  {source.label}
                </a>
              </li>
